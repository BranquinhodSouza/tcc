...
A Educação de Jovens e Adultos (EJA) constitui uma modalidade de ensino fundamental para a promoção da inclusão social e do resgate da cidadania de sujeitos historicamente marginalizados do processo educacional brasileiro. Conforme dados do Anuário Brasileiro da Educação Básica \cite{todos2025}, o acesso à EJA caiu 34,5\% na última década, registrando 2,4 milhões de matrículas em 2024, o menor número da série histórica. Paralelamente, 29\% dos brasileiros de 15 a 64 anos são analfabetos funcionais, evidenciando a persistência de um grave déficit educacional.
...
A Educação de Jovens e Adultos (EJA) no Brasil constitui uma modalidade educativa amparada pela Lei de Diretrizes e Bases da Educação Nacional (LDB 9.394/96), que em seu art. 37 estabelece que a educação de jovens e adultos será destinada àqueles que não tiveram acesso ou oportunidade de estudos no ensino fundamental e médio na idade própria.

Historicamente, a EJA no Brasil emerge de um contexto de exclusão social e educacional. Conforme \citeonline{dipierro2005}, a modalidade se constituiu como uma política compensatória, voltada para reparar uma dívida histórica com a população trabalhadora. A autora argumenta que a EJA oscila entre a função de ``reparação'' e a de ``educação permanente'', sendo frequentemente relegada a segundo plano nas políticas educacionais brasileiras.

O pensamento de \citeonline{freire2023} é fundamental para compreender a EJA sob uma perspectiva crítica e emancipadora. Freire propõe uma educação dialógica, que parte da realidade vivida pelos educandos, valorizando seus saberes prévios e experiências de vida. Para Freire, a educação de adultos não pode ser uma mera transferência de conhecimento, mas um ato de conhecimento onde educador e educando aprendem juntos, na problematização do mundo. Essa abordagem contrapõe-se ao que o autor chama de ``educação bancária'', onde o aluno é visto como um recipiente vazio a ser preenchido pelo professor.

A EJA atende majoritariamente um público marcado pela diversidade e pela vulnerabilidade social. Dados do Anuário Brasileiro da Educação Básica \cite{todos2025} indicam que 76,8\% dos alunos da EJA são pretos ou pardos, e a maioria possui renda familiar de até 1,5 salários mínimos. Esse perfil demográfico evidencia a interseção entre desigualdade racial e educacional no Brasil, tema explorado por \citeonline{arroyo2017}, que destaca a necessidade de políticas específicas para os ``sujeitos da EJA''.

\subsection{O PROEJA como Política de Integração}

O Programa Nacional de Integração da Educação Profissional com a Educação Básica na Modalidade de Educação de Jovens e Adultos (PROEJA) foi instituído para superar a histórica separação entre educação geral e formação profissional no Brasil. De acordo com \citeonline{frignotto2005}, a integração entre educação profissional e educação básica visa superar a dualidade educacional brasileira, que historicamente ofereceu uma educação acadêmica para as elites e uma educação profissional aligeirada para as classes trabalhadoras.

Entretanto, a implementação do PROEJA tem enfrentado inúmeros desafios. \citeonline{pereira2011} aponta que o programa foi implantado de forma compulsória nas instituições federais, sem a devida preparação institucional e sem a participação efetiva dos docentes na construção dos projetos político-pedagógicos.

\subsection{Evasão Escolar na EJA e no PROEJA}

A evasão escolar é um fenômeno complexo e multidimensional, definido por \citeonline{tinto1975} como o processo de abandono do curso ou instituição antes da conclusão. No contexto da EJA, a evasão assume contornos específicos, sendo frequentemente associada a fatores externos ao ambiente escolar.

\citeonline{oliveira2021}, em sua revisão integrativa sobre a evasão no PROEJA, identificaram duas grandes categorias de causas: (1) fatores relacionados ao trabalho, remuneração e benefícios sociais; e (2) fatores relacionados à formação e prática docente. A primeira categoria reflete a realidade do aluno-trabalhador, que muitas vezes precisa optar entre o estudo e a sobrevivência. A segunda categoria revela a inadequação das práticas pedagógicas tradicionais para atender as especificidades dos estudantes jovens e adultos.

\citeonline{nery2020} investigaram as mediações pedagógicas em turmas de EJA e constataram que predomina uma prática de ensino alicerçada na didática tradicional (``bancária''), em detrimento de uma abordagem contextualizada e dialógica. Os autores observaram que o número de professores preparados para atuar na EJA é insuficiente, e que a capacitação específica para a modalidade é rara.

\citeonline{santos2025} analisam os desafios da EJA no século XXI, destacando a permanência de metodologias pedagógicas inadequadas, a descontinuidade das políticas públicas e os efeitos do envelhecimento populacional. \citeonline{tolentino2025} reforça que o fortalecimento da EJA requer o comprometimento do poder público, da sociedade civil e da comunidade acadêmica na construção de políticas permanentes e eficazes.

\subsection{Estudos sobre o PROEJA no IFG Campus Goiânia}

\citeonline{pereira2011} desenvolveu um estudo de caso sobre o PROEJA no IFG Campus Goiânia, investigando os fatores de acesso e permanência na escola. A pesquisa revelou que os alunos do PROEJA são majoritariamente trabalhadores, com idade entre 18 e 40 anos, que buscam no programa tanto a conclusão da educação básica quanto a qualificação profissional. Os principais fatores de evasão identificados foram: incompatibilidade entre horário de trabalho e aula, cansaço físico, dificuldades de aprendizagem e falta de apoio familiar.

\citeonline{castro2016} analisaram o percurso contraditório do PROEJA no IFG Campus Goiânia, apontando que o programa foi implementado de forma focalizada, com adesão inicial de apenas uma coordenação de área (Turismo e Hospitalidade). As autoras argumentam que a resistência institucional ao PROEJA reflete uma visão elitista da educação profissional. Em sua dissertação, \citeonline{castro2018} aprofunda a análise das contradições, limites e perspectivas, examinando os cursos Técnicos Integrados em Serviços de Alimentação e Cozinha.

\subsection{Análise de Dados Educacionais e Mineração de Dados}

A análise de dados educacionais tem se consolidado como área promissora para compreensão dos fenômenos educacionais. \citeonline{assis2017} aplicou técnicas de mineração de dados para criar perfis de estudantes com propensão à evasão no ensino superior brasileiro, utilizando algoritmos de classificação como CART e \textit{Random Forest}. O estudo identificou que as principais características dos alunos com maior risco de evasão incluem: ingressar no primeiro semestre, possuir vínculos com mais de uma instituição e ter notas acima da média no ENEM.

A utilização de \textit{dashboards} para monitoramento educacional tem se expandido como ferramenta de apoio à gestão acadêmica. Segundo \citeonline{few2006}, \textit{dashboards} são displays visuais das informações mais importantes, consolidadas em uma única tela para monitoramento rápido. No contexto educacional, \textit{dashboards} de evasão permitem que gestores identifiquem precocemente alunos em risco e avaliem o impacto de políticas de assistência estudantil.
...
\midrule
Freire & 2023 & Educação dialógica e emancipadora para adultos \\
Di Pierro & 2005 & Políticas públicas e identidade da EJA \\
Arroyo & 2017 & Sujeitos da EJA e direitos educacionais \\
...
Nery, Pereira \& Amaral & 2020 & Mediações pedagógicas na EJA \\
